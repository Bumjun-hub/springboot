...
public class MemberController {
	...
	public String selectByNameLikeNameSort(@RequestParam("name") String name, Model model) {
		...
		String name2 = "%" + name + "%";
		/*
		 * Sort / Sort.Order - Spring Framework 일부, 데이터 정렬을 지정하는데 사용 - Sort 클래스는 하나이상의
		 * Sort.Order 객체를 가지고 있음 ex) 2개 이상일 때 Sort sort = Sort.by(
		 * Sort.Order.desc("name"), Sort.Order.asc("email") );
		 */
